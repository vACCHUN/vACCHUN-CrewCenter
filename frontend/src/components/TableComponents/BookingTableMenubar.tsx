...
function BookingTableMenubar({
  selectedDate,
  setSelectedDate,
  sidebarOpen,
  setSidebarOpen,
}: BookingTableMenubarParams) {
      ...
      <div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-y-4 md:gap-y-0">
          ...
          <div className="flex justify-center md:justify-end px-4 md:px-3 md:col-span-2 text-center md:text-right">
            ...
            >
              {DAYS[convertToDate(selectedDate).getDay()]} -{" "}
        ...
        <div className="flex items-center justify-end py-1 px-3 ">
          <div className="relative group mr-2">
            <i
              onClick={() => setSidebarOpen(sidebarOpen ? false : "exams")}
              className={`cursor-pointer fa-solid fa-user-graduate ${sidebarOpen ? "text-blue-500" : "text-awesomecolor"}`}
            ></i>
            <span className="tooltip top-8 h-min">CPTs</span>
          </div>
        </div>
      </div>
      {eventDetailsOpen && currentEvent.event != null ? (
        <EventDetails
          event={currentEvent.event}
          message={currentEvent.message}
          closeModal={() => setEventDetailsOpen(false)}
        />
      ) : (
        <></>
      )}
