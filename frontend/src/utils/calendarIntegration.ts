...
export function generateAppleCalendarICS(startDate: string, endDate: string, startHour: number, startMinute: number, endHour: number, endMinute: number, sector: string, subSector: string) {
  ...
  const icsContent = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//vACCHUN//CrewCenter//EN", "BEGIN:VEVENT", `UID:${crypto.randomUUID()}@vacchun`, `DTSTAMP:${formatUTCForICS(new Date())}`, `DTSTART:${formatUTCForICS(startDateUTC)}`, `DTEND:${formatUTCForICS(endDateUTC)}`, `SUMMARY:${title}`, `DESCRIPTION:vACCHUN Booking: ${sector}/${subSector}\\nCreated by vACCHUN CrewCenter`, "END:VEVENT", "END:VCALENDAR"].join("\r\n");
