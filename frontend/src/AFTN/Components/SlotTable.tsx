...
function SlotTable() {
  ...
  const getLhbpData = async () => {
    try {
      ...
      const data: IFPS[] = import.meta.env.DEV ? res.data : res.data.filter((data: IFPS) => (data.ctot.trim() !== "" || data.atfcmStatus == "DES" || data.cdmSts == "SUSP" || data.atfcmStatus == "SLC") && data.atot.trim() === "");
      console.log(res);
          ...
          {lhbpData
            ...
            .map((data) => (
              <SlotTableEntry
                ...
                ctot={data.ctot}
                taxiTime={data.taxi}
                mostPenalizing={data.mostPenalizingAirspace}
                key={data.callsign}
              />
            ))}
